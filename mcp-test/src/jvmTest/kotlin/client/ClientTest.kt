...
import io.modelcontentprotocol.kotlin.sdk.client.ClientOptions
...
class ClientTest {
    @Test
    fun `should initialize with matching protocol version`() = runBlocking {
    ...
    @Test
    fun `should initialize with supported older protocol version`() = runBlocking {
    ...
    @Test
    fun `should reject unsupported protocol version`() = runBlocking {
        var closed = false
        val clientTransport = object : AbstractTransport() {
            override suspend fun start() {}
    ...
    @Test
    fun `should respect server capabilities`() = runBlocking {
    ...
    @Test
    fun `should respect client notification capabilities`() = runBlocking {
        val server = Server(
            Implementation(name = "test server", version = "1.0"),
    ...
    @Test
    fun `should respect server notification capabilities`() = runBlocking {
        val server = Server(
            Implementation(name = "test server", version = "1.0"),
            ServerOptions(
                capabilities = ServerCapabilities(
    ...
    @Test
    fun `should handle client cancelling a request`() = runBlocking {
        val server = Server(
            Implementation(name = "test server", version = "1.0"),
            ServerOptions(
    ...
    @Test
    fun `should handle request timeout`() = runBlocking {
        val server = Server(
            Implementation(name = "test server", version = "1.0"),
            ServerOptions(
                capabilities = ServerCapabilities(resources = ServerCapabilities.Resources(listChanged = null, subscribe = null))
    ...
    @Test
    fun `should only allow setRequestHandler for declared capabilities`() = runBlocking {
        val client = Client(
            clientInfo = Implementation(
                name = "test client",
                version = "1.0"
    ...
    @Test
    fun `JSONRPCRequest with ToolsList method and default params returns list of tools`() = runBlocking {
        val serverOptions = ServerOptions(
            capabilities = ServerCapabilities(
                tools = ServerCapabilities.Tools(null)
            )
